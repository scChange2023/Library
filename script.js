...
//Update Library Log
let totalBooks = document.querySelector(".bookCount");
let readBooksDiv = document.querySelector("#readCount");
let notReadBooksDiv = document.querySelector("#notReadCount");
let books = 0;

function filterReadBooks(obj) {
  if (obj.read === true) {
    return true;
  }
}
...
function updateLibraryLog() {
  ...
  let readBooks = myLibrary.filter(function (el) {
    return el.read === true;
  });

  let notReadBooks = myLibrary.filter(function (el) {
    return el.read === false;
  });

  readBooksDiv.textContent = readBooks.length;
  notReadBooksDiv.textContent = notReadBooks.length;
}
...
function createBook() {
  ...
  toggleInput.classList.add("input_click");
  ...
  if (rowInputArray[5] === true) {
    ...
    book.classList.add("notReadColor");
  }
  //Reset RowInput Array for next book
  rowInputArray = [];
  //Update Library Log
  updateLibraryLog();
  alert("Adam smells");
...
bookContainer.addEventListener("click", function (event) {
  ...
  //Check Color
  if (event.target.classList.contains("input_click")) {
    ...
    switch (bookCard.classList.contains("readColor")) {
      case true:
